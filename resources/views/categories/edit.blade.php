<x-app-layout>
    ...
    <div class="my-8 mx-8 ">
        <form class="flex flex-col gap-3" method="POST" action="{{ route('categories.update', $category->id) }}">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <textarea id="description" name="description" required
                    ...
                    placeholder="Opis proizvoda...">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
